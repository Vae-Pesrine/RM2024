...
int main(int argc, char** argv)
{
  ...
  nhPrivate.getParam("gazebo_cmd_vel_p", gazebo_cmd_vel_p);
  ...
  // ros::Publisher pub_gazebo_vel = nh.advertise<geometry_msgs::Twist> ("/cmd_vel",5);

  geometry_msgs::TwistStamped cmd_vel;
  cmd_vel.header.frame_id = "vehicle";

  //gazebo_cmd_vel
  // geometry_msgs::Twist gazebo_cmd_vel;
    ...
    if (pathInit) {
      ...
      if (pubSkipCount < 0) {
        ...
        if (fabs(vehicleSpeed) <= maxAccel / 100.0) {
          ...
          //
        //  if(fabs(yawDiff) > 1.57) vehicleSpeed *= 0.8;
        ...
        cout<<"YawSpeed"<<vehicleYawRate<<endl;
